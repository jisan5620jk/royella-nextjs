...
const Page = ({ bookingsData }) => {
  ...
  useEffect(() => {
    if (isClient) {
      ...
      const guest = params.get("guestName");
  ...
  const setAlert = () => {
    Swal.fire({
      title: "Are you sure?",
      text: `Do you want to book the room: ${roomName}?`,
      icon: "warning",
  ...
  const checkInDate = bookingsData?.selectedInDate
    ? new Date(bookingsData.selectedInDate)
    : new Date();
  const checkOutDate = bookingsData?.selectedOutDate
    ? new Date(bookingsData.selectedOutDate)
    : new Date(new Date().setDate(new Date().getDate() + 3));

  const adult = bookingsData?.adult ?? "2";
  const children = bookingsData?.children ?? "1";
  const room = bookingsData?.room ?? "1";
            ...
            <div>
              <div className=" bg-whiteSmoke dark:bg-normalBlack px-7 py-8 md:px-8 md:py-10 lg:px-9 lg:py-11 2xl:px-10 2xl:pt-[45px] 2xl:pb-[30px] grid-flow-row-dense">
                ...
                >
                  <div className="bg-white dark:bg-lightBlack h-10 lg:h-[50px] 2xl:h-[56px] grid items-center justify-start px-3 sm:px-5 2xl:px-6">
                    <p className="text-sm md:text-[15px] leading-[26px] font-Lora font-medium text-lightBlack dark:text-white">
                      ...
                      <span className="text-khaki">
                        {checkInDate.toDateString().slice(4)}
                      </span>
                    </p>
                  </div>
                  <div className="bg-white dark:bg-lightBlack h-10 lg:h-[50px] 2xl:h-[56px] grid items-center justify-start px-3 sm:px-5 2xl:px-6">
                  ...
                  <div className="bg-white dark:bg-lightBlack h-10 lg:h-[50px] 2xl:h-[56px] grid items-center justify-start px-3 sm:px-5 2xl:px-6">
                    <p className="text-sm md:text-[15px] leading-[26px] font-Lora font-medium text-lightBlack dark:text-white">
                      Adult - <span className="text-khaki">{adult}</span>
                    </p>
                  </div>
                  <div className="bg-white dark:bg-lightBlack h-10 lg:h-[50px] 2xl:h-[56px] grid items-center justify-start px-3 sm:px-5 2xl:px-6">
                    <p className="text-sm md:text-[15px] leading-[26px] font-Lora font-medium text-lightBlack dark:text-white">
                      Children - <span className="text-khaki">{children}</span>
                    </p>
                  </div>
                  <div className="bg-white dark:bg-lightBlack h-10 lg:h-[50px] 2xl:h-[56px] grid items-center justify-start px-3 sm:px-5 2xl:px-6">
                    <p className="text-sm md:text-[15px] leading-[26px] font-Lora font-medium text-lightBlack dark:text-white">
                      Rooms - <span className="text-khaki">{room}</span>
                    </p>
                  </div>
                </div>
              </div>
